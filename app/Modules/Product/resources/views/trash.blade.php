...
<div class="d-flex justify-content-end">
            <a class="btn btn-primary float-right" href="#" id="restore">Restore All</a>
            <a class="btn btn-danger float-right" href="{{ route('product.index') }}" role="button">Cancel</a></br>

        </div>
    <!-- {{-- <a class="btn btn-danger float-right"  href="{{ route('colour.showtrash') }}" role="button">restore</a> --}}
    <a class="btn btn-danger float-right" href="{{ route('colour.index') }}" role="button">Cancel</a></br>
...
<table id="myTable" class="display table table-bordered table-striped">
                            ...
                            <label class="switch">
                             ...
                             <td>
                                <div>
                                    ...
                                    <a href="{{ route('product.index') }}"class="delete" data-id="{{ $col->id }}"><i class="fas fa-undo"></i>        </a>
                                </div>
                             </td>


                                {{-- <a href="{{ route('product.index3') }}"class="delete"  onclick="event.preventDefault();
                                    document.getElementById('#delete-form--{{$col->id}}').submit();"></a>
                                     <form id="delete-form-{{$col->id}}" action="{{ route('product.delete',$col->id) }}" method="POST" style="display: none;">
                                        {{ csrf_field() }}
                                        @method('DELETE')
                                    </form> --}}
   ...
   <script>
    ...
    $('#restore').click(function (e) {
        ...
        $.ajax({
            type: "POST",
            url: "/admin/product/restoretrash",
            // data: "data",
            // dataType: "dataType",
